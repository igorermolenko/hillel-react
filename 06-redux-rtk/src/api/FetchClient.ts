...
export class FetchClient<T> {
    ...
    async getAll(): Promise<T[]> {
        try {
            ...
            throw Error(`Can't fetch list of waiters from server: ${e.message}`)
        }
    }
    ...
    async create(data: T): Promise<T> {
        try {
            ...
            throw Error(`Can't create waiter: ${e.message}`)
        }
    }
    ...
    async update(id: number, data: T): Promise<T> {
        try {
            ...
            throw Error(`Can't update waiter: ${e.message}`)
        }
    }
    ...
    async delete(id: number): Promise<void> {
        try {
            ...
            throw Error(`Can't delete waiter: ${e.message}`)
        }
    }
}